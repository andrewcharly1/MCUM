...
def test_generate_tests_for_skill_uses_full_budget(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda skill_name, limit=0, min_confidence=0.0: [
            _failure_pattern(index)
            | {
    ...
    assert all(item["safety_contract"] == "switch_strategy_before_retry" for item in adversarial)
