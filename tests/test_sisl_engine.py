...
def test_generate_adversarial_tests_uses_failures_runs_and_degraded(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert tests[0]["safety_contract"] == "warn_before_action"
    assert tests[1]["safety_contract"] == "switch_strategy_before_retry"
    assert tests[2]["safety_contract"] == "revalidate_before_reuse"


def test_generate_adversarial_tests_parses_stringified_json_content(monkeypatch: pytest.MonkeyPatch) -> None:
    cursor = _CursorStub(
        [
            [
                {
                    "id": "fp-json",
                    "title": "Stringified content",
                    "task_description": "Retry a stale plan",
                    "content": "{\"conclusion\": \"Revalidate before retrying.\"}",
                    "current_confidence": 0.8,
                }
            ]
        ]
    ...
    tests = adversarial_test_generator.generate_adversarial_tests("mcum-orchestrator", limit=1)

    assert len(tests) == 1
    assert tests[0]["expected_result"] == "Advertir el riesgo: Revalidate before retrying."
    assert tests[0]["risk_hint"] == "failure_pattern"
...
def test_skill_writer_is_idempotent_for_same_proposals(tmp_path: Path) -> None:
    skill_dir = tmp_path / "sample-skill"
    skill_dir.mkdir()
    skill_md = skill_dir / "SKILL.md"
    original = "# Sample Skill\n\n## Rules\n\nKeep it deterministic.\n"
    ...
    first = skill_writer.apply_sisl_proposals(
        "sample-skill",
        [proposal],
        skill_md_path=str(skill_md),
    )
    second = skill_writer.apply_sisl_proposals(
        "sample-skill",
        [proposal],
        skill_md_path=str(skill_md),
    )
    ...
    assert updated.count(skill_writer.SISL_WRITEBACK_START) == 1
    assert updated.count("Warn before mutating production data.") == 1
...
def test_get_skills_for_evaluation_prioritizes_low_test_count_and_stale_cycles(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    cursor = _CursorStub(
        [
            [
                {"skill_name": "skill-a", "n": 10},
                {"skill_name": "skill-b", "n": 10},
                {"skill_name": "skill-c", "n": 2},
            ],
            [
                {"skill_name": "skill-a", "n": 8},
                {"skill_name": "skill-b", "n": 2},
                {"skill_name": "skill-c", "n": 2},
            ],
            [
                {"skill_name": "skill-a", "last_sisl_at": "2026-03-27T10:00:00+00:00"},
                {"skill_name": "skill-b", "last_sisl_at": "2026-03-25T10:00:00+00:00"},
            ],
        ]
    )

    monkeypatch.setattr(autonomous_loop, "discover_local_skills", lambda: [
        {"skill_name": "skill-c"},
        {"skill_name": "skill-a"},
        {"skill_name": "skill-b"},
    ])
    monkeypatch.setattr(autonomous_loop, "get_db", lambda: _ConnManager())
    monkeypatch.setattr(autonomous_loop, "get_cursor", lambda conn: _CursorManager(cursor))

    ordered = autonomous_loop.get_skills_for_evaluation()

    assert ordered == ["skill-b", "skill-c", "skill-a"]
...
def test_live_execution_policy_enables_writeback_for_target_skills() -> None:
    # Locks in the user's choice: SISL auto-rewrites SKILL.md for the three
    # target skills (with backup + CKL gate). If someone disables it in the
    # live execution_policy.json, this fails loudly.
    from MCUM.policy import load_execution_policy

    policy = load_execution_policy()
    assert str(policy.get("autonomous_writeback", "disabled")) != "disabled"
    for skill in ("html-dashboard-expert", "go-industrial-expert", "ui-ux-pro-max"):
        assert autonomous_loop.resolve_writeback_mode(skill, policy) != "disabled", skill
    # A non-target skill stays gated.
...
def test_run_autonomous_improvement_disables_writeback_for_non_targeted_skill(
    ...
    assert result["phase"] == "sisl_cycle"
    assert result["signal_summary"]["phase"] == "sisl_cycle"
    assert result["signal_summary"]["accepted"] is None


def test_run_autonomous_improvement_builds_skip_signal_summary(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    stats = {
        "experience_count": 5,
        ...
        "last_sisl_at": datetime.now(timezone.utc).isoformat(),
    }

    monkeypatch.setattr(autonomous_loop, "load_execution_policy", lambda: {"autonomous_writeback": "disabled"})
    monkeypatch.setattr(autonomous_loop, "get_skill_loop_stats", lambda skill_name: stats)
    monkeypatch.setattr(autonomous_loop, "get_current_skill_version", lambda skill_name: "1.0.0")
    monkeypatch.setattr(autonomous_loop, "run_sisl_cycle", lambda *args, **kwargs: pytest.fail("run_sisl_cycle should not be called on cooldown"))

    result = autonomous_loop.run_autonomous_improvement(
        skill_name="mcum-orchestrator",
        trigger="manual_test",
        verbose=False,
        config=autonomous_loop.AutonomousLoopConfig(cooldown_minutes=60),
    )

    assert result["skipped"] is True
    assert result["reason"] == "cooldown_active"
    assert result["signal_summary"]["phase"] == "cooldown"
    assert result["signal_summary"]["next_step"] == "wait for cooldown and retry"
    assert result["signal_summary"]["test_count"] == 7
