...
MCUM_PARENT = MCUM_ROOT.parent
REQ_FILE = MCUM_ROOT / "requirements.txt"
GRAPH_REQ_FILE = MCUM_ROOT / "requirements-graph.txt"
...
def install_dependencies() -> None:
    ...
    if GRAPH_REQ_FILE.exists():
        run_checked([sys.executable, "-m", "pip", "install", "-r", str(GRAPH_REQ_FILE)])
        ok("Native graph intelligence dependencies installed")
...
def verify_connection() -> None:
    ...
    sys.path.insert(0, str(MCUM_PARENT))
    ...
    if status["pgvector"]:
        ...
        if status.get("pgvector_installed"):
            ...
            from MCUM.db.experience_store import _is_pgvector_enabled
...
def backfill_embeddings() -> None:
    ...
    sys.path.insert(0, str(MCUM_PARENT))
    from MCUM.db.embedder import warmup_model
    from MCUM.db.experience_store import compute_and_store_missing_embeddings
