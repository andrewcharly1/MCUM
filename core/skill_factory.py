...
def collect_skill_gap_signals(
    *,
    ...
    signals.sort(
        key=lambda item: (item["occurrences"], item["failure_count"], -item["avg_confidence"]),
        reverse=True,
    )
    return signals
...
def _normalize_dispatch_hints(
    ...
    *,
    ...
    for skill_name, hint in hints_by_skill.items():
        triggers = _dedupe_strings(hint["triggers"])
        anti = _dedupe_strings(hint["anti"])
...
def review_testing_skill_versions(
    *,
    ...
    for version in versions:
        ...
        if should_roll_back:
            skill_md_path = Path(record["skill_path"]) / "SKILL.md"
...
def filter_dispatchable_skills(
    ...
    *,
    ...
    for skill in registry:
        ...
        item = dict(skill)
        metadata = dict(record.get("metadata") or {})
        dispatch_hints = dict(metadata.get("dispatch_hints") or {})
        item["triggers"] = _dedupe_strings(
            list(item.get("triggers", [])) + list(dispatch_hints.get("triggers") or [])
        )
        item["anti"] = _dedupe_strings(
            list(item.get("anti", [])) + list(dispatch_hints.get("anti") or [])
        )
