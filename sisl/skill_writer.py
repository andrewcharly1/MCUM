...
def _proposal_signature(proposals) -> str:
    payload = []
    for proposal in proposals:
        payload.append(
            {
                "type": proposal.improvement_type,
                "section": _normalize_section_key(proposal.section_to_edit or ""),
                "text": proposal.proposed_text.strip(),
                "confidence": round(float(proposal.confidence), 3),
                "tests": sorted(list(proposal.test_ids_failing or [])),
            }
        )
...
def _dedupe_proposals(proposals) -> list:
    deduped = []
    seen: set[tuple[str, str, str]] = set()
    for proposal in proposals:
        key = (
            str(proposal.improvement_type),
            _normalize_section_key(str(proposal.section_to_edit or "")),
            str(proposal.proposed_text or "").strip(),
        )
        if key in seen:
            continue
        seen.add(key)
...
def build_candidate_block(skill_name: str, proposals, *, section_id: str, section_title: str) -> str:
    ...
    lines = [
        ...
        "",
        f"Proposal signature: {proposal_signature}",
...
def _replace_existing_block(text: str, section_id: str, block: str) -> tuple[str, str] | None:
    ...
    if text[start_index:end_index] == block:
        return text, "noop"
...
def _insert_into_existing_section(text: str, section_title: str, block: str) -> tuple[str, str] | None:
    ...
    lines = text.splitlines(keepends=True)
    prefix = "".join(lines[: match.end_line]).rstrip("\n")
    suffix = "".join(lines[match.end_line:]).lstrip("\n")
    updated = prefix + "\n\n" + block.strip() + "\n\n" + suffix
    return updated, "insert_section"
...
def apply_sisl_proposals(skill_name: str, proposals, skill_md_path: str | None = None) -> dict:
    ...
    proposal_list = _dedupe_proposals(list(proposals))
    ...
    if updated == original:
        return {
            "applied": False,
            "note": "SKILL.md ya contenia el mismo writeback SISL; no se realizaron cambios.",
