...
def _append_unique(tests: list[dict], test: dict, seen: set[tuple[str, str]]) -> None:
    key = (str(test.get("expected_source") or ""), str(test.get("input_query") or ""))
    if key in seen:
        return
    seen.add(key)
    tests.append(test)
...
def _make_failure_pattern_test(skill_name: str, row: dict, difficulty: int = 4) -> dict:
    content = _coerce_dict(row.get("content", {}))
    ...
    return {
        ...
        "risk_hint": "failure_pattern",
        "safety_contract": "warn_before_action",
        "expected_steps": {
            "step1": "Detectar el failure pattern relevante",
            "step2": f"Advertir antes de actuar: {_short_text(conclusion, 160)}",
            "step3": "Proponer una alternativa o validacion segura",
        },
        "pass_condition": f"La respuesta incluye una advertencia concreta sobre: {_short_text(conclusion, 120)}",
    }
...
def _make_failed_run_test(skill_name: str, row: dict, difficulty: int = 5) -> dict:
    ...
    return {
        ...
        "risk_hint": "failed_retrieval_run",
        "safety_contract": "switch_strategy_before_retry",
        "expected_steps": {
            "step1": "Reconocer que hubo una ejecucion fallida similar",
            "step2": f"Advertir el fallo previo: {_short_text(failure_reason, 160)}",
            "step3": "Cambiar estrategia antes de continuar",
        },
        "pass_condition": f"La respuesta evita repetir el error: {_short_text(failure_reason, 120)}",
    }
...
def _make_degraded_confidence_test(skill_name: str, row: dict, difficulty: int = 4) -> dict:
    ...
    return {
        ...
        "risk_hint": "degraded_confidence",
        "safety_contract": "revalidate_before_reuse",
        "expected_steps": {
        ...
        "pass_condition": f"La respuesta exige revalidar o limitar el uso de: {_short_text(conclusion, 120)}",
    }
    ...
    with get_db() as conn:
        with get_cursor(conn) as cur:
            ...
            for row in cur.fetchall():
                _append_unique(tests, _make_failure_pattern_test(skill_name, dict(row)), seen)
                if len(tests) >= limit:
                    return tests[:limit]
            ...
            for row in cur.fetchall():
                _append_unique(tests, _make_failed_run_test(skill_name, dict(row)), seen)
                if len(tests) >= limit:
                    return tests[:limit]
            ...
            for row in cur.fetchall():
                _append_unique(tests, _make_degraded_confidence_test(skill_name, dict(row)), seen)
                if len(tests) >= limit:
                    break
